...
pub fn discover_request_variables(
    ...
    visit_request_strings(request, |value, usage| {
...
fn visit_request_strings<E>(
    request: &HttpRequest,
    mut visit: impl FnMut(&str, &VariableUsage) -> Result<(), E>,
) -> Result<(), E> {
    if let Some(method) = &request.method {
        visit(method, &VariableUsage::Method)?;
    }
    if let Some(url) = &request.url {
        visit(url, &VariableUsage::Url)?;
    }
    for header in &request.headers {
        let usage = VariableUsage::Header {
            name: header.name.clone(),
        };
        visit(&header.name, &usage)?;
        visit(&header.value, &usage)?;
    }
    for parameter in &request.query_parameters {
        let usage = VariableUsage::QueryParameter {
            name: parameter.name.clone(),
        };
        visit(&parameter.name, &usage)?;
        visit(&parameter.value, &usage)?;
    }
    ...
    if let RequestProtocol::Graphql(Some(body)) = &request.protocol {
        visit_graphql_body(body, &mut visit)?;
    }
    if let Some(authentication) = &request.authentication {
        for (name, value) in &authentication.properties {
            let usage = VariableUsage::Authentication { name: name.clone() };
            visit_authentication_value(value, &usage, &mut visit)?;
        }
    }
    Ok(())
}

fn visit_graphql_body<E>(
    body: &GraphqlBody,
    visit: &mut impl FnMut(&str, &VariableUsage) -> Result<(), E>,
) -> Result<(), E> {
    match body {
        GraphqlBody::Single(operation) => visit_graphql_operation(operation, visit),
        GraphqlBody::Variants(variants) => {
            for variant in variants {
                visit_graphql_operation(&variant.body, visit)?;
            }
            Ok(())
        }
    }
...
fn visit_graphql_operation<E>(
    operation: &GraphqlOperation,
    visit: &mut impl FnMut(&str, &VariableUsage) -> Result<(), E>,
) -> Result<(), E> {
    if let Some(query) = &operation.query {
        visit(query, &VariableUsage::GraphqlQuery)?;
    }
    if let Some(variables) = &operation.variables {
        visit_json_strings(variables, &VariableUsage::GraphqlVariables, visit)?;
    }
    if let Some(operation_name) = &operation.operation_name {
        visit(operation_name, &VariableUsage::GraphqlOperationName)?;
    }
    if let Some(extensions) = &operation.extensions {
        visit_json_strings(extensions, &VariableUsage::GraphqlExtensions, visit)?;
    }
    Ok(())
}

fn visit_json_strings<E>(
    ...
    fn visit_value<E>(
        ...
        match value {
            serde_json::Value::String(value) => visit(value, usage),
            serde_json::Value::Array(values) => {
                for value in values {
                    visit_value(value, usage, visit)?;
                }
                Ok(())
            }
...
fn visit_body<E>(
    ...
    match body {
        RequestBody::Single(body) => visit_body_value(body, visit),
        RequestBody::Variants(variants) => {
            for variant in variants {
                visit_body_value(&variant.body, visit)?;
            }
            Ok(())
        }
    }
...
fn visit_body_value<E>(
    body: &Body,
    visit: &mut impl FnMut(&str, &VariableUsage) -> Result<(), E>,
) -> Result<(), E> {
    match body {
        Body::Raw(body) => visit(&body.data, &VariableUsage::Body)?,
        Body::FormUrlEncoded(fields) => {
            for field in fields {
                let usage = VariableUsage::FormUrlEncoded {
                    name: field.name.clone(),
                };
                visit(&field.name, &usage)?;
                visit(&field.value, &usage)?;
            }
        }
        Body::Multipart(parts) => {
            for part in parts {
                ...
                visit(&part.name, &usage)?;
                match &part.value {
                    MultipartValue::Single(value) => visit(value, &usage)?,
                ...
                if let Some(content_type) = &part.content_type {
                    visit(content_type, &usage)?;
                }
            }
        }
        Body::File(files) => {
            for file in files {
                visit(&file.file_path, &VariableUsage::File)?;
                visit(&file.content_type, &VariableUsage::File)?;
            }
        }
    }
    Ok(())
...
fn visit_authentication_value<E>(
    ...
    match value {
        ...
        AuthenticationValue::Sequence(values) => {
            for value in values {
                visit_authentication_value(value, usage, visit)?;
            }
        }
