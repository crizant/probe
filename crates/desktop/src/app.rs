...
impl DesktopMenuItem {
    fn action(label: &'static str, action: impl Action + 'static) -> Self {
        Self::Action(label, Box::new(action), None)
    }

    fn checked_action(label: &'static str, checked: bool, action: impl Action + 'static) -> Self {
        Self::Action(label, Box::new(action), Some(checked))
    }

    fn submenu(label: &'static str, state: DesktopSubmenu, popup: DesktopMenuDefinition) -> Self {
